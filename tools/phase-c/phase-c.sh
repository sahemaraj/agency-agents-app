...
run_windows_vm() {
  ...
  run_step "windows pe header verification" prlctl exec "$WINDOWS_VM" cmd.exe /c "call \"$WINDOWS_BUILD_TOOLS\\VC\\Auxiliary\\Build\\vcvarsall.bat\" x64 & dumpbin /headers C:\\AgencyAgentsWinTarget\\release\\shikigami.exe | findstr /C:\"AA64 machine\" & dumpbin /headers C:\\AgencyAgentsWinTargetX64\\x86_64-pc-windows-msvc\\release\\shikigami.exe | findstr /C:\"8664 machine\""
  run_step "windows named artifacts" prlctl exec "$WINDOWS_VM" cmd.exe /c "if not exist C:\\AgencyAgentsWinTarget\\release\\shikigami.exe exit /B 1 & if not exist C:\\AgencyAgentsWinTargetX64\\x86_64-pc-windows-msvc\\release\\shikigami.exe exit /B 1 & if not exist C:\\AgencyAgentsArtifacts mkdir C:\\AgencyAgentsArtifacts & copy /Y C:\\AgencyAgentsWinTarget\\release\\shikigami.exe \"C:\\AgencyAgentsArtifacts\\Shikigami-0.1.0-windows-arm64.exe\" & copy /Y C:\\AgencyAgentsWinTargetX64\\x86_64-pc-windows-msvc\\release\\shikigami.exe \"C:\\AgencyAgentsArtifacts\\Shikigami-0.1.0-windows-x64.exe\" & dir C:\\AgencyAgentsArtifacts"
