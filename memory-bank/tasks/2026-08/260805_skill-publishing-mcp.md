...
Complete revision-bound Skill draft publication through MCP and make the Skills workspace popovers
dismiss when the user clicks outside them.
...
- `src/lib/components/SkillsWorkspace.svelte` — approval label and outside-click dismissal.
...
- Live UI: inside clicks preserved each popover; outside clicks closed Manage Sources and Approval
  Inbox; opening one closed the other.
