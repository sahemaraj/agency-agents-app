...
pub async fn run_skip(state: &AppState, version: &str) -> Result<(), AppError> {
    use crate::commands::settings::{load_async, persist, SettingsLoadState};
    ...
    let mut settings_cache = state.settings.write().await;
    if let SettingsLoadState::Corrupt { message } = &*settings_cache {
    ...
    let updated_settings = {
        match load_async(&state.app_data_dir).await {
            SettingsLoadState::Loaded(mut s) => {
    ...
    *settings_cache = SettingsLoadState::Loaded(clamped);
    drop(settings_cache);
